...
public extension UIView {
    ...
    public func addSubview<T: UIView>(view: T, insets: UIEdgeInsets = UIEdgeInsets.zero) {
        view.translatesAutoresizingMaskIntoConstraints = false
        addSubview(view)
        let key = String.random(ofLength: 10)
        addConstraints(
            NSLayoutConstraint.constraints(withVisualFormat: "H:|-\(insets.left)-[\(key)]-\(insets.right)-|", options: NSLayoutFormatOptions(rawValue: 0), metrics: nil, views: ["\(key)": view])
        )
        addConstraints(
            NSLayoutConstraint.constraints(withVisualFormat: "V:|-\(insets.top)-[\(key)]-\(insets.bottom)-|", options: NSLayoutFormatOptions(rawValue: 0), metrics: nil, views: ["\(key)": view])
        )
    }
